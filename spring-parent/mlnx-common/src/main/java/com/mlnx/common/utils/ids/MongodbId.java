package com.mlnx.common.utils.ids;
 ...
 */
public class MongodbId implements Comparable<MongodbId>, Serializable {

    private static final long serialVersionUID = 3670079982654483072L;

    static final MyLog LOGGER = MyLog.getLog("MongodbId");
     ...
     * @return the new id
     */
    public static MongodbId get() {
        return new MongodbId();
    }

    /**
     * Checks if a string could be an {@code MongodbId}.
     *
     * @param hexString a potential MongodbId as a String.
    ...
    /**
     * <p>Creates an MongodbId using time, machine and inc values.  The Java driver used to create all IdTests this way, but it does not
     * match the <a href="http://docs.mongodb.org/manual/reference/object-id/">MongodbId specification</a>, which requires four values, not
     ...
     *
     * <p>NOTE: This will not break any application that use IdTests.  The 12-byte representation will be round-trippable from old to new
     * driver releases.</p>
     *
     * @param time    time in seconds
     * @param machine machine ID
     * @param inc     incremental value
     * @return a new {@code MongodbId} created from the given values
     * @since 2.12.0
     */
    public static MongodbId createFromLegacyFormat(final int time, final int machine, final int inc) {
        return new MongodbId(time, machine, inc);
    }
     ...
     * @param date the date
     */
    public MongodbId(final Date date) {
     ...
     */
    public MongodbId(final Date date, final int counter) {
     ...
     */
    public MongodbId(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(dateToTimestampSeconds(date), machineIdentifier, processIdentifier, counter);
    }

    /**
     * Creates an MongodbId using the given time, machine identifier, process identifier, and counter.
     *
     ...
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not zero
     */
    public MongodbId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private MongodbId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter,
     ...
     * @throws IllegalArgumentException if the string is not a valid hex string representation of an MongodbId
     */
    public MongodbId(final String hexString) {
     ...
     * @since 3.4
     */
    public MongodbId(final ByteBuffer buffer) {
        ...
        // and MongodbId's are always in big-endian order.
     ...
     *
     * @return a string representation of the MongodbId in hexadecimal format
     */
    ...
    public boolean equals(final Object o) {
        ...
        MongodbId objectId = (MongodbId) o;
    ...
    @Override
    public int compareTo(final MongodbId other) {
        if (other == null) {
    ...
    private static byte[] parseHexString(final String s) {
        if (!isValid(s)) {
            throw new IllegalArgumentException("invalid hexadecimal representation of an MongodbId: [" + s + "]");
        }
